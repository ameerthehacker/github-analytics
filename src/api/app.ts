...
import { GetUsernameResponse, SetupUserRequest, SetupUserResponse } from './contract';
...
connectToDatabase().then((orm) => {
  ...
  routes.post('/setup', async (req: Request<{}, {}, SetupUserRequest>, res: Response<SetupUserResponse>) => {
    ...
    try {
      const user = new User(fullName, createHash('sha512').update(password, 'utf8').digest('hex'));
